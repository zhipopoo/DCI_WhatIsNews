...
// Upload operations have no timeout — large files may take a while
...
export const initChunkedUpload = (data: InitChunkedUploadRequest) =>
  request.post<ApiResult<{ uploadId: string }>>('/admin/media/upload/init', data, { timeout: UPLOAD_TIMEOUT }).then(res => res.data);
...
export const uploadChunk = (uploadId: string, chunkIndex: number, chunk: Blob) => {
  ...
  return request.post<ApiResult<{ uploadId: string; chunkIndex: number; received: boolean }>>(
    '/admin/media/upload/chunk', formData,
...
export const completeChunkedUpload = (uploadId: string) =>
  request.post<ApiResult<MediaFile>>('/admin/media/upload/complete', { uploadId }, { timeout: UPLOAD_TIMEOUT }).then(res => res.data);

/** Get chunked upload status (for resuming) */
export const getChunkedUploadStatus = (uploadId: string) =>
  request.get<ApiResult<ChunkedUploadStatus>>(`/admin/media/upload/status/${uploadId}`, { timeout: UPLOAD_TIMEOUT }).then(res => res.data);

/** Cancel a chunked upload and clean up temp files */
export const cancelChunkedUpload = (uploadId: string) =>
  request.delete<ApiResult<void>>(`/admin/media/upload/cancel/${uploadId}`, { timeout: UPLOAD_TIMEOUT }).then(res => res.data);
